...
-- ViewEra (converted into x, y, z scale)
local xScale = config.ViewEra.x or 3.8
local yScale = config.ViewEra.y or 3.8
local zScale = config.ViewEra.z or 3.8

-- Calculate vertical and horizontal FOV based on the x, y, z values
local fovVertical = (yScale / 3.8) * 0.7
local fovHorizontal = (xScale / 3.8) * 0.5
...
local function isWithinViewEra(position)
    ...
    return (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude <= math.sqrt((fovVertical / 2)^2 + (fovHorizontal / 2)^2)
end
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    -- Add range check before firing to ensure the target is close enough to the predicted position
                    local mouseX, mouseY = getMousePosition()
                    local distanceToTarget = (Vector2.new(mouseX, mouseY) - Vector2.new(screenPos.X, screenPos.Y)).Magnitude
                    if onScreen and isWithinViewEra(predictedPos) and distanceToTarget < 50 then
